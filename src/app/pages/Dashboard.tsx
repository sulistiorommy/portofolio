...
export function Dashboard() {
  ...
  const [wakaStats, setWakaStats] = useState<WakaTimeData | null>(null);
  const [umamiStats, setUmamiStats] = useState<UmamiStats | null>(null);
  const [umamiLocations, setUmamiLocations] = useState<UmamiMetricPoint[]>([]);
  const [umamiPages, setUmamiPages] = useState<UmamiMetricPoint[]>([]);
  const [umamiReferrers, setUmamiReferrers] = useState<UmamiMetricPoint[]>([]);
  const [umamiChartData, setUmamiChartData] = useState<UmamiChartPoint[]>([]);
  ...
  const VisitTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ value: number; payload: { x?: string; day?: string } }> }) => {
  ...
  const WakaTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ value: number; payload: { date?: string; day?: string } }> }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-slate-900 p-3 border border-slate-200 dark:border-slate-700 shadow-xl rounded-xl">
          <p className="text-xs font-bold text-slate-400 uppercase mb-1">{payload[0].payload.date || payload[0].payload.day}</p>
          <p className="text-sm font-black text-emerald-500">{payload[0].value}h</p>
        </div>
      );
    }
    return null;
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={wakaStats?.activity || []} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  <XAxis
                    ...
                    dy={10}
                  />
                  <Tooltip content={<WakaTooltip />} cursor={{ fill: 'transparent' }} />
                  <Bar
                    dataKey="hours"
                    radius={[6, 6, 0, 0]}
                    barSize={40}
                  >
                    {wakaStats?.activity?.map((_entry, index: number) => (
                      <Cell
                        key={`cell-${index}`}
